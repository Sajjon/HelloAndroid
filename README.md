...
TASKS:
0) When signing up the user only needs to input the password field once
1) Change colors of the background and or buttons. You can do this in three different ways, of which two relates to each other.
i) Programmatically, with Java code in the activity <---- NOT RECOMMENDED
ii) In the layout file
	a) Using the "Text" tab, showing "RAW" XML <---- RECOMMENDED (but old school)
	b) Using the Design tab <---- NOT RECOMMENDED
